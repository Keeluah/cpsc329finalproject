...
public class panelGameRound extends JPanel {
	...
	public panelGameRound(JPanel mainGUI, ArrayList<pwdStruct> levels, double[] times, int round, int totalLevels) {
		...
		int key = levels.get(round-1).getKey();
		String keystr;
		if(key > 0) keystr = "+" + Integer.toString(key);
		else keystr = Integer.toString(key);
		JLabel lblKey = new JLabel("Key used for encrypton: " + keystr);
